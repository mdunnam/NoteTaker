...
export default async function ReviewPage() {
  ...
  const [lowConfidenceNotes, reclassificationCandidates, thinkingMemory] = await Promise.all([
  ...
  const totalReviewItems = lowConfidenceNotes.length + reclassificationCandidates.length;
    ...
    <div className="p-6">
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 md:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-xs text-gray-600">All active clarification and regrouping work.</p>
        </div>
      ...
      {totalReviewItems === 0 ? (
        ...
        <div className="space-y-8">
          ...
          <section className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-5">
            <h2 className="text-base font-semibold text-gray-900">Next In Review</h2>
            <p className="mt-2 text-sm text-gray-600">
              Forgotten-note resurfacing and repeated-pattern review will land here next, so review becomes the single queue for memory payoff instead of a collection of scattered widgets.
            </p>
          </section>
        </div>
      )}
